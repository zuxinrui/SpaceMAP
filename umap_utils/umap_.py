...
import umap_utils.distances as dist

import umap_utils.sparse as sparse

from umap_utils.utils import (
    submatrix,
    ts,
    csr_unique,
    fast_knn_indices,
...
from umap_utils.layouts import (
...
# TODO: calculate the space distortion:
# @numba.njit(
#     locals={
...
#     fastmath=True,
# )
# def calculate_space_distortion_rate(D, dim):
...
def smooth_knn_dist_exp2(distances, k, n_iter=64, local_connectivity=1.0, bandwidth=1.0):  # n_iter=64
    ...
    for i in range(distances.shape[0]):
        ...
        for n in range(n_iter):
            ...
            for j in range(1, distances.shape[1]):
                d = distances[i, j]
                if d > 0:
                    psum += np.exp(-(d ** 2 / mid))  # TODO: 这里似乎已经计算了probability，可以再优化一下
                else:
                    psum += 1.0
...
def compute_membership_strengths_exp2(
    knn_indices, knn_dists, sigmas, rhos, return_dists=False, bipartite=False,
):
    ...
    for i in range(n_samples):
        for j in range(n_neighbors):
            ...
            if (bipartite == False) & (knn_indices[i, j] == i):
                val = 0.0
            elif knn_dists[i, j] <= 0.0 or sigmas[i] == 0.0:
                val = 1.0
            else:
                # *******************************************************************************
                # ******************************** main function ********************************
                # *******************************************************************************
                val = np.exp(-((knn_dists[i, j] ** 2) / (sigmas[i])))
...
def compute_membership_strengths_v2(
    knn_indices, knn_dists, sigmas, rhos, bayes_rate, return_dists=False, bipartite=False,
):
    n_samples = knn_indices.shape[0]
    n_neighbors = knn_indices.shape[1]
    ...
    for i in range(n_samples):
        for j in range(n_neighbors):
            ...
            if (bipartite == False) & (knn_indices[i, j] == i):
                ...
                # ******************************** main function ********************************
                # *******************************************************************************
                val = np.exp(-((knn_dists[i, j] - rhos[i] + bayes_rate * np.take(Pj, knn_indices[i])[j]) / (sigmas[i])))
...
def fuzzy_simplicial_set(
    ...
    if apply_set_operations:
        ...
        # 这一步非常关键，直接把local 1-simplicial set 变成了global，一定要理解
        # fuzzy union: G = A + A^T - AAT
        result = (
...
def fuzzy_simplicial_set_exp2(
    ...
    # numba: This step already compute the P matrix inplicitly, TODO: extract this P matrix
    # only use knn_dists
    sigmas, rhos = smooth_knn_dist_exp2(
        knn_dists, float(n_neighbors), local_connectivity=float(local_connectivity),
    )

    # numba:
    # use knn_dists and knn_indices
    rows, cols, vals, dists = compute_membership_strengths_exp2(
        knn_indices, knn_dists, sigmas, rhos, return_dists
    )
...
def fuzzy_simplicial_set_bayes(
    X,
    n_neighbors,
    random_state,
    metric,
    ...
    if apply_set_operations:
        ...
        Pj = np.sum(knn_dists, axis=1)
        Pj = np.exp(-Pj)
        # Pj = np.power(Pj / np.sum(Pj) * Pj.shape[0], -1)
        Pj = np.tile(Pj, (Pj.shape[0], 1))
        result = result * Pj
        result = scipy.sparse.coo_matrix(result)
...
# TODO: 这种bayes成分占比多少，应该继续研究
def fuzzy_simplicial_set_bayes_in_threshold(
    X,
    n_neighbors,
    random_state,
    metric,
    ...
    Dj = np.sum(knn_dists, axis=1)
    Dj = np.tile(Dj, (1, knn_dists.shape[0]))
    knn_dists_bayes = knn_dists + bayes_rate * np.take(Dj, knn_indices)
    # print(knn_dists_bayes)

    # # TODO: very inefficient
    # for i in range(knn_dists.shape[0]):
    #     knn_dists_bayes[i] = knn_dists[i] + 0.01*np.take(Dj, knn_indices[i])

    # numba: This step already compute the P matrix inplicitly, TODO: extract this P matrix
    sigmas, rhos = smooth_knn_dist_bayes(
        knn_dists_bayes, float(n_neighbors), local_connectivity=float(local_connectivity),
    )

    # numba:
    rows, cols, vals, dists = compute_membership_strengths_v2(
        knn_indices, knn_dists, sigmas, rhos, bayes_rate, return_dists
    )
...
def fuzzy_simplicial_set_t_distribution(
    X,
    n_neighbors,
    random_state,
    metric,
    ...
    # numba: This step already compute the P matrix inplicitly, TODO: extract this P matrix
    sigmas, rhos = smooth_knn_dist_t_dist(
        knn_dists, float(n_neighbors), local_connectivity=float(local_connectivity),
    )

    # numba:
    rows, cols, vals, dists = compute_membership_strengths_t_dist(
    ...
    # fuzzy union: G = A + A^T - AAT
    result = (
            1.0 * (result + transpose - prod_matrix)
            + (1.0 - 1.0) * prod_matrix
    )

    # # P = knn_dists - np.tile(rhos.reshape(rhos.shape[0], -1), (1, n_neighbors))
    # P[:, 0] = NPY_INFINITY
    # P += 1.
    # P = np.power(P, (1.0 + 1.0) / -2.0)  # 1.0: DoF
    # result = np.maximum(P, MACHINE_EPSILON)  # 这里面不应该包括对角元素，因为原论文公式就没带
    # result = result.toarray()
...
def simplicial_set_embedding(
    ...
    if isinstance(init, str) and init == "random":
        ...
        pca = PCA(n_components=n_components, svd_solver='randomized',
                  random_state=random_state)
        initialisation = pca.fit_transform(data).astype(np.float32, copy=False)
    ...
    elif isinstance(init, str) and init == "precomputed":
        print('[SpaceMAP] init precomputed!')
        embedding = precompute_data
    else:
    ...
    if euclidean_output:
        embedding, intermediate_embedding, intermediate_data = optimize_layout_euclidean(
            embedding,
            embedding,
            head,
            tail,
    ...
    return embedding, aux_data, intermediate_embedding, init_data, intermediate_data
...
class UMAP(BaseEstimator):
    ...
    def __init__(
        ...
        self.intermediate_embedding = 0
        self.init_embedding = 0
        self.intermediate_data = 0
        self.precomputed_init = precomputed_init
    ...
    def _validate_parameters(self):
        ...
        if isinstance(self.init, str) and self.init not in ("spectral", "random", "pca", "precomputed"):
            raise ValueError('string init values must be "spectral" or "random" or "pca"')
        if (
    ...
    def fit(self, X, y=None):
        ...
        if self.transform_mode == "embedding":
            self.embedding_, aux_data, self.intermediate_embedding, self.init_embedding, self.intermediate_data = self._fit_embed_data(
    ...
    def transform(self, X):
        ...
        if self.output_metric == "euclidean":
            embedding, _ = optimize_layout_euclidean(
                embedding,
