...
def _optimize_layout_euclidean_single_epoch_spacemap(
    head_embedding,
    tail_embedding,
    head,
    tail,
    ...
    for i in numba.prange(epochs_per_sample.shape[0]):
        if epoch_of_next_sample[i] <= n:
            ...
            for p in range(n_neg_samples):
                ...
                if dist_squared > 0.0:
                    grad_coeff = 2.0 * gamma / (beta*factor) * (1/(1-np.exp(factor*pow(dist_squared, (1/beta)))) - 1)
                elif j == k:
                    continue
                else:
                    grad_coeff = 0.0
...
def _optimize_layout_euclidean_single_epoch(
    ...
    update_positive = np.zeros(dim)
    update_negative = np.zeros(dim)
    for i in numba.prange(epochs_per_sample.shape[0]):
        if epoch_of_next_sample[i] <= n:
            j = head[i]
            k = tail[i]
            ...
            for d in range(dim):
                ...
                update_positive[d] = grad_d * alpha

                current[d] += update_positive[d]
                if move_other:
                    other[d] += -update_positive[d]
            ...
            for p in range(n_neg_samples):
                k_negative = tau_rand_int(rng_state) % n_vertices

                other = tail_embedding[k_negative]
                ...
                for d in range(dim):
                    ...
                    update_negative[d] += grad_d * alpha

            ans = np.append(j, update_positive + update_negative)
            ans = np.append(ans, k)
            update_per_iter[i] = np.append(ans, -update_positive)
    ...
    # print('one epoch complete', update_per_iter)
...
def optimize_layout_euclidean(
    ...
    update_per_iter = np.zeros((epochs_per_sample.shape[0], 2 + 2*dim))
    intermediate_embedding = np.zeros((n_epochs, epochs_per_sample.shape[0], 2 + 2*dim))
    intermediate_data = np.zeros((n_epochs, head_embedding.shape[0], dim))
    ...
    for n in range(n_epochs):
        ...
        intermediate_embedding[n] = update_per_iter
        intermediate_data[n] = head_embedding
    ...
    return head_embedding, intermediate_embedding, intermediate_data
